 ...
 * Controller 
 * Pagina de control "employee_files_system_personal"
 */
...
var NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC                      = '';
...
var NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER     = '';
...
var FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC                      = '';
...
var FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER     = '';
...
function cleanGeneralVariables() {
	...
	NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC                      = '';
	...
	NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER     = '';
	...
	FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC                      = '';
	...
	FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER     = '';
...
function respFindAll(data){
	...
	if(data){
		var tableEmployee =	  ' <table id="bootstrap-data-table"				'
							...
							+ ' 		<th>Título/Cédula profesional                             	</th>                          '
							+ ' 		<th>CURP                                                    </th>                          '
							+ ' 		<th>RFC                                                     </th>                          '
							+ ' 		<th>IMSS                                                    </th>                          '
							+ ' 		<th>Infonavit                                               </th>                          '
		...
		for (var i = 0; i < data.length; i++){
			...
			if(rowEmployee.enabled) {
				...
				enabledDelete = '<i class="fa fa-trash-o btn btn-secondary" title="No existe Archivos Personales"></i>';
			} else {
			...
			var actions =
				...
				'<i class="fa fa-pencil btn btn-warning" title="Editar" onclick="editEmployee(\'' 
										...
										+ '\'' + rowEmployee.recommendationLetter		+ '\''
			...
			// -----------------------------------------------------------
			if(rowEmployee.recommendationLetter) {
				tr += '<td>' 
						+ '<i class="fa fa-download btn btn-info" title="Ver o descargar documento" onclick="viewDocActa(\'' 
								+ rowEmployee.id + '\','
								+ '\'' + rowEmployee.name + ' ' + rowEmployee.lastName 	+ '\','
				...
				tr += '<td>'+ rowEmployee.recommendationLetter  +'</td>';
			}
			// -----------------------------------------------------------
...
function hideViewAddEmployee() {
	...
	$('#btn-view-recommendationLetter').css('display', 'none');
}
...
function saveEmployee() {
	...
	//
	if(NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC) {
		// Editar
		if(castNameFileToTypeFile('rfc')) {
			employeefilessystempersonal.rfc	= castNameFileToTypeFile('rfc');
		} else {
			employeefilessystempersonal.rfc	= NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC;
		}
	} else {
		// Guardar
		employeefilessystempersonal.rfc	= castNameFileToTypeFile('rfc');
	}
	employeefilessystempersonal.fileRfc	= FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC;
	//
	...
	//
	if(NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER) {
		// Editar
		if(castNameFileToTypeFile('recommendationLetter')) {
			employeefilessystempersonal.recommendationLetter	= castNameFileToTypeFile('recommendationLetter');
		} else {
			employeefilessystempersonal.recommendationLetter	= NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER;
		}
	} else {
		// Guardar
		employeefilessystempersonal.recommendationLetter	= castNameFileToTypeFile('recommendationLetter');
	}
	employeefilessystempersonal.fileRecommendationLetter	= FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER;
	//
	// -----------------------------------------------------------------------------------------------------------
	//
...
function confirmDeleteEmployee(name, id, active) {
    $.jAlert({
        'type': 'confirm',
        'title': 'Confirmación',
        'confirmQuestion': '¿ Seguro de eliminar información archivos personales a </br>' + name + ' ?',
        'confirmBtnText': 'Si',
...
function activeEmployee(id, active) {
	...
	sendPostAction(EMPLOYEE_GRAL_CONTROLLER + 'enabledUserEmployeeFilesSystemPersonal/' + idEmployee + '/' + active, null, respActiveEmployeeFilesSystemPersonal);
}

function respActiveEmployeeFilesSystemPersonal(data) {
	showPreloader(false);
	if(data && data[1] == 'Ok'){
...
function editEmployee(id, name, 
	...
	NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC                     = rfc;
	...
	NAME_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER	= recommendationLetter;
	//
	var splitId = id.split('_');
	if(splitId.length == 2) {
		ID_EMPLOYEE_GRAL = splitId[0];
	...
	if (rfc) {
		// $('#btn-view-rfc').css('display', 'block');
	} else {
		$('#btn-view-rfc').css('display', 'none');
	}
	if (imss) {
	...
	if (recommendationLetter) {
		// $('#btn-view-recommendationLetter').css('display', 'block');
	} else {
		$('#btn-view-recommendationLetter').css('display', 'none');
	}
...
function setValueFile(inputFile) {
	...
	if (inputFile == 'rfc') {
		...
		var readerFiles = new FileReader();
		readerFiles.onloadend = function() {
			FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RFC = readerFiles.result;
		}
		if (local) {
			readerFiles.readAsDataURL(local);
		} else {
	...
	if (inputFile == 'recommendationLetter') {
		var local = $('#' + inputFile)[0].files[0];
		var readerFiles = new FileReader();
		readerFiles.onloadend = function() {
			FILE_EMPLOYEE_FILES_SYSTEM_PERSONAL_RECOMMENDATIONLETTER = readerFiles.result;
		}
		if (local) {
			readerFiles.readAsDataURL(local);
		} else {
